...
import { adminLoginSchema, entryPasscodeOnlySchema, entrySchema, observerLoginSchema, participantLoginSchema, selfCheckinSchema, signupSchema, tournamentSettingsSchema, } from "../schemas.js";
import { nextPowerOfTwo, shuffle } from "../services/tournamentService.js";
// 管理者パスコードを発行
async function issueUniqueAdminPasscode(deps) {
    for (let i = 0; i < 8; i++) {
...
export function registerAuthRoutes(app, deps) {
    app.post("/api/auth/signup", async (req, res) => {
        ...
        if (existing)
            return res.status(400).json({ error: "このメールアドレスは既に登録済みです。" });
    ...
    app.post("/api/auth/admin-login", async (req, res) => {
        ...
        res.cookie("nomiteni_token", deps.auth.createToken({ userId: user.id, role: UserRole.ADMIN, tournamentId: tournament.id }), {
            httpOnly: true,
            sameSite: "lax",
        });
